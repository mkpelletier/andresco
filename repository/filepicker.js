...
M.core_filepicker.init = function(Y, options) {
    ...
    Y.extend(FilePickerHelper, Y.Base, {
        ...
        build_tree: function(node, level) {
            ...
            if(node.children) {
            	// BEGIN: Andresco
            	// Fix HTML being displayed when viewing as list by not applying italics/underline
        ...
        view_files: function(page) {
            ...
            // BEGIN: Andresco
            // Default Andresco to view as list as user will generally be browsing through
            // quite a few files and the view as icon format can become cumbersome.
            if (this.active_repo.repo_type == 'andresco') {
                this.view_as_smaller_icons();
                return;
            }
            // END: Andresco            
	   ...
	   // BEGIN: Andresco
	   // View as smaller icons when viewing as icons as the default icons make
	   // file browsing with lots of files cumbersome.
       view_as_smaller_icons: function() {
            var scope = this;
            var client_id = this.options.client_id;
            var list = this.filelist;
            var panel_id = '#panel-'+client_id;
            ...
            for(var k in list) {
                ...
                var filename_id = 'filname-link-'+client_id+'-'+String(count);
				// Reduce font size for Andresco
                title.innerHTML += '<a href="###" id="'+filename_id+'" title="'+node.title+'"><span style="font-size:90%">'+filename+"</span></a>";
				...
				grid.style.height = title.style.height = '100%';
                ...
                if(node.children) {
                    ...
                    var fileinfo = {};
                    fileinfo['title'] = list[k].title;
                    fileinfo['usertitle'] = list[k].usertitle;
                    fileinfo['description'] = list[k].description;                    
                    fileinfo['source'] = list[k].source;
                    fileinfo['thumbnail'] = list[k].thumbnail;
                    fileinfo['haslicense'] = list[k].haslicense?true:false;
                    fileinfo['hasauthor'] = list[k].hasauthor?true:false;
		...
		set_moodle_form_fields: function(filename, title, description) {
            ...
            // Otherwise Description field is a text area
                ...
                description_field.set('value', description);
            }
        ...
        select_file: function(args) {
            ...
            // BEGIN: Andresco
			// TODO: Language pack
            html += '<tr><td class="mdl-right"><label for="newname-'+client_id+'">File name:</label></td>';            
            if (this.active_repo.repo_type == 'andresco') {
				// Increase size of the input box for the file title nd disable edting
				html += '<td class="mdl-left"><input type="text" id="newname-'+client_id+'" value="'+args.title+'" size=50/></td></tr>';
				
				// Add Alfresco title field (which can be modified)
				// Note in Andresco the Alfresco "title" is referred to as usertitle
				// TODO: Use language pack for description below
				html += '<tr><td class="mdl-right"><label for="newname-'+client_id+'">'+'Title'+':</label></td>';
				if (args.usertitle == null) {
					// Use filename if the title is not set.
					args.usertitle = args.title;
				}
				html += '<td class="mdl-left"><input type="text" name="title" id="newname-'+client_id+'" value="' + args.usertitle + '" size="50"/></td></tr>';

				// Add Alfresco description field (which can be modified)
				// TODO: Use language pack for description below
            ...
            if ((this.options.externallink && this.options.env == 'editor' && this.options.return_types != 1)) {
            	// BEGIN: Andresco
                if (!this.active_repo.repo_type == 'andresco') {
            	// Only display external link option if content access method is copy. Hide for link.
            	if (this.active_repo.page == 100) {
            		// By default make the link external checkbox checked regardless of content access method
                	html += '<tr'+le_style+'><td></td><td class="mdl-left"><input type="checkbox" id="linkexternal-'+client_id+'" value="" checked="yes" '+le_checked+' />'+M.str.repository.linkexternal+'</td></tr>';
               	}
                }
                // END: Andresco
            }
            ...
            // BEGIN: Andresco
            var filename = Y.one('#newname-'+client_id).get('value');
            this.set_moodle_form_fields(filename, args.usertitle, args.description)
            // END: Andresco
        },
        wait: function(type) {
        ...
        parse_repository_options: function(data) {
            ...
            // Used by custom code to determine if dealing with an andresco repository
            this.active_repo.repo_type = this.options.repositories[this.active_repo.id].type;			
			var lastlocation = YAHOO.util.Cookie.get('lastlocation');
        ...
        create_upload_form: function(data) {
            ...
            // BEGIN: Andresco
            if (this.active_repo.repo_type == 'andresco') {
            	// TODO: Language pack this
	            str += '<label for="'+id+'_file">Filename:</label></td>';            	
            }
            else {
            ...
            // BEGIN: Andresco
            if (this.active_repo.repo_type == 'andresco') {
            str += '<tr><td class="mdl-right"><label for="newname-'+client_id+'">'+'Title'+':</label></td>';
            str += '<td class="mdl-left"><input type="text" name="title" id="newname-'+client_id+'" value="" size="50" /></td></tr>';
            }else {
            ...
            // BEGIN: Andresco
            if (this.active_repo.repo_type == 'andresco') {
            // Include the ability to specify a description for the file being uploaded.  
            // TODO: Use language pack for description below          
                str += '<td class="mdl-right"><label for="newdescription-'+client_id+'">'+'Description'+':</label></td>';
                str += '<td class="mdl-left"><textarea name="description" id="newdescription-'+client_id+'" rows="5" cols="40"/></textarea></td></tr>';
            str += '<tr>';
            // Allow user to make a comment on any given version of an upload (including initial upload)
            // TODO: Use language pack for version comment label below            
    			str += '<td class="mdl-right"><label for="newcomment-'+client_id+'">'+'Version Comment'+':</label></td>'; 
				// Default version comment to user & time
            ...
            str += '</tr>';
            // BEGIN: Andresco
            // Hide the license field in Andresco
            if (this.active_repo.repo_type == 'andresco') {
            	str += '</table>';
            }
            else {
            ...
            // BEGIN: Andresco
            if (this.active_repo.repo_type == 'andresco') {
                // TODO: change below to use language string from Andresco instead of hardcoding here
            if (this.options.env == 'url' || this.options.env == 'editor') {
            	button_text = 'Upload and Select File';
            }
            else {
            	button_text = 'Upload File';
            }
            str += '<div class="fp-upload-btn"><button id="'+id+'_action">'+button_text+'</button></div>';
            }
            else {
            ...
            Y.one('#'+id+'_action').on('click', function(e) {
                ...
                if (!Y.one('#newname-'+client_id).get('value')) {
                	// TODO: Language pack
	                scope.print_msg("Please specify a title", 'error');
	                return false;
                }
                var title = Y.one('#newname-'+client_id).get('value');
        ...
        print_header: function() {
                    ...
                    scope.request({
                        ...
                        callback: function(id, obj, args) {
                            ...
                            search_dialog = new YAHOO.widget.Dialog("fp-search-dlg", {
                               ...
                               width : "30em",
                               // BEGIN: Andresco
                               // The below causes problems with the modal dialog and makes it unusable
                               //modal: true,
                               // END: Andresco
                               fixedcenter : true,
                               zindex: 9999991,
            ...
            if(!r.norefresh) {
                ...
                refresh.on('click', function() {
                    // BEGIN: Andresco
                    if (this.active_repo.repo_type == 'andresco') {
                    // Pass the repository we are in so the refresh is context specific
                    current_path = this.filepath[this.filepath.length-1];
                    this.list(current_path);
                    }
                    // END: Andresco
                }, this);
            ...
            // BEGIN: Andresco
            // Display the upload file button in the toolbar
            if(!r.upload && this.active_repo.repo_type == 'andresco') {         	
            	var label = "Upload File";
            	var html = '<a href="###"><img src="'+M.util.image_url('t/adddir')+'" /> '+label+'</a>';
             	var upload = Y.Node.create(html);
            ...
            if(r.manage) {
                ...
                mgr.target = "_blank";
                // BEGIN: Andresco
                // Rename "Manage" to "Go to Repo"
                // TODO: Find a way to get this into alfresco repository language pack
                if (this.active_repo.repo_type == 'andresco') {
                mgr.innerHTML = '<img src="'+M.util.image_url('a/setting')+'" /> '+ "Go To Repo";
                }
                else {
                    mgr.innerHTML = '<img src="'+M.util.image_url('a/setting')+'" /> '+ M.str.repository.manage;   
                }
                // END: Andresco
